...
### OP-F28 status update — complex transform and limiting law CLOSED

The complex extension is now proved.
...
=
\lim_T
T^{-1}\sum_{r<T}\mathfrak S(H_{2^rh})^z
\]

exists locally uniformly for every \(z\in\mathbb C\), and is entire.
...
converge weakly to a unique probability law \(\nu_h\) with

\[
\phi_h(t)=\mathcal M_h(it).
\]

The full transform is its bilateral Laplace transform,

\[
\mathcal M_h(z)=\int e^{zy}\,d\nu_h(y),
\]

and \(\nu_h\) is the Haar pushforward of the measurable phase field on the compact procyclic hull generated by the base-4 clocks \(e_p=\operatorname{ord}_p(4)\).

The whole law is dyadically invariant:

\[
\nu_{2^kh}=\nu_h.
\]

### OP-F29 — distribution type and cumulant geometry

The next gate is to characterize \(\nu_h\), not merely prove its existence.

Priority questions:

1. determine whether \(\nu_h\) has atoms, a continuous component, or a singular-continuous component;
2. determine whether the law is nondegenerate for every admissible \(h\);
3. compute the analytic cumulant-generating germ
   \[
   K_h(z)=\log\mathcal M_h(z)
   \]
   near \(z=0\), and identify its derivatives with the previously proved phase cumulants;
4. derive rigorous Chernoff/large-deviation bounds from the entire moment transform;
5. determine whether the procyclic hull representation gives a useful decomposition into finite-order resonance components;
